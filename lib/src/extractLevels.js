...
const USAGE = `Usage: node src/extractLevel.js <file.ifc>
EXAMPLE
...
async function extractLevels() {
  let ifcFilename = 'IFCFilename'
  if (process.argv.slice(2)[0] !=null) {
    ifcFilename = getIFCFileName()
    const relElementsGrouped = await extractRELID(ifcFilename)
    await createManipulateNewIFC(relElementsGrouped, ifcFilename)
  } else {
    console.log('No IFC File defined')
  }
}

/** Extract ifc filename from command line
 * @return {string} ifc filename
 */
function getIFCFileName() {
 ...
 * @param {string} ifcFilename extracted ifc filename
 */
 ...
 * @param {string} ifcFilename extracted ifc filename
 */
export function createManipulateNewIFC(relatedElemGrouped, ifcFilename) {
 ...
 * @param {bool} log - Shows logs of creation and manipulation
 */
function removeLinesFromIFCwithExpressID(filename, expressIDsToRemove, log = true) {
  ...
  if (fileCopiedBool) {
    fs.readFile(filename, 'utf8', (err, data) => {
      ...
      fs.writeFile(filename, removeLines(data, linesToRemove), 'utf8', function(err) {
        ...
        if (log) console.log(filename+' is ready')
      })
    })
  } else {
 ...
 */
export function copyFilesPromise(IFCfilename, newfilename, expressIDsToRemoveIFCRel, log=true) {
  return new Promise((resolve, reject)=>{
    fs.copyFile(IFCfilename, newfilename, (err) => {
      if (err && log) {
        ...
        if (log) console.log('File '+newfilename+' is created')
        resolve(true)
        removeLinesFromIFCwithExpressID(newfilename, expressIDsToRemoveIFCRel, log)
      }
    })
  })
}
