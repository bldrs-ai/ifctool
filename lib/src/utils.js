...
/**
 * @param {string} simpleCsv Tokens separated by commas, no escaped commas.
 * @return {object}
 */
export function simpleCsv2Json(simpleCsv) {
  return JSON.parse(`["${simpleCsv.split(',').join('","')}"]`)
}
...
export function jsonToCsv(json, omitEmptyFields = false, formatOpts = null) {
  const formatFields = formatOpts ? simpleCsv2Json(formatOpts) : null
  const transforms = [
    (item) => {
      if (omitEmptyFields) {
